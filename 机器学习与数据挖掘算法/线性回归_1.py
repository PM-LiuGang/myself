...
for model in model_dic:#读取每个回归模型对象
    #交叉模型中做训练，scores各个模型的交叉得分,cross val score就 
...
#利用pyecharts可视化 
#超过2条以上的折线，还是用交互式的图 比较好
from pyecharts import Line
line = Line()
line.add('BR',list(range(x.shape[0])),pre_y_list[0],line_type='dashed',line_width=1)
line.add('LR',list(range(x.shape[0])),pre_y_list[1],line_type='dotted',line_width=2)
line.add('ETC',list(range(x.shape[0])),pre_y_list[2],line_width=3)
line.add('SVR',list(range(x.shape[0])),pre_y_list[3],line_type='dotted',line_width=4)
line.add('GBR',list(range(x.shape[0])),pre_y_list[4],line_type='dashed',line_width=5)
