...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def query_ticketing_analysis_click(self):
        ...
        stations = eval(read('stations.txt'))  # 读取所有车站并转换为dic类型
    ...
    def query_time_click(self):
        ...
        stations_time = eval(read('time.txt'))  # 读取所有车站与起售时间并转换为dic类型
        stations = eval(read('stations.txt'))  # 读取所有车站并转换为dic类型
...
if __name__ == '__main__':
    if is_stations('stations.txt') == False and is_stations('time.txt') == False:
        get_station()
        get_selling_time()
    if is_stations('stations.txt') == True and is_stations('time.txt') == True:
        show_MainWindow()
    else:
